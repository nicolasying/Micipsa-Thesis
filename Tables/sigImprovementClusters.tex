...
% \end{tablenotes}  
\end{ThreePartTable}
\caption[\code{SIG} Voxel Improvement Clusters]{[TODO] We thresholded Wilcoxon signed-rank test's significance at \(10^{-4.35}\) as a clean cut is found in p-value histogram, which leads to a selection of top .5\% important voxel-model improvements. The largest and most improved voxel-cluster is found in left BA21, then in right Angular Gyrus which is part of Wernicke's area (language, V. S. Ramachandran, and Edward Hubbard published a paper in 2003 metaphores). A more lateral and smaller-cluster improvement is found in right MTG. \label{tab:sigImprovementClusters}}
\end{table}